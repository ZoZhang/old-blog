<?php
/*
	Plugin Name: WP-Cumulus中文汉化
	Plugin URI: http://www.popvan.cn/?p=464
	Description: Flash based Tag Cloud for WordPress
	Version: 2.0
	Author: Kisazbis
	Author URI: http://www.doomeve.com
...
function wp_cumulus_add_pages() {
	add_options_page('WP Cumulus中文汉化', 'WP Cumulus中文汉化', 8, __FILE__, 'wp_cumulus_options');
}
...
function wp_cumulus_createflashcode( $widget=false, $atts=NULL ){
	...
	if( $options['compmode']!='true' ){
		...
		$flashtag .= '<p>WP-Cumulus by <a href="http://www.doomeve.com/">Kisazbis</a> blog <a href="http://www.doomeve.com/">末日黄昏</a> or <a href="http://doomeve.com">末日黄昏</a></p>';
...
function wp_cumulus_options() {	
	...
	echo "<div class=\"wrap\"><h2>显示设置</h2>";
	echo '<table class="form-table">';
	// width
	echo '<tr valign="top"><th scope="row">插件宽度</th>';
	echo '<td><input type="text" name="width" value="'.$options['width'].'" size="5"></input><br />单位为:px (建议设为:250或者更大)</td></tr>';
	// height
	echo '<tr valign="top"><th scope="row">插件高度</th>';
	echo '<td><input type="text" name="height" value="'.$options['height'].'" size="5"></input><br />单位为:px (最好设置为宽度的3/4)</td></tr>';
	// text color
	echo '<tr valign="top"><th scope="row">标签初始颜色</th>';
	echo '<td><input type="text" name="tcolor" value="'.$options['tcolor'].'" size="8"></input> 逐渐加深为: <input type="text" name="tcolor2" value="'.$options['tcolor2'].'" size="8"></input>  鼠标选中颜色: <input type="text" name="hicolor" value="'.$options['hicolor'].'" size="8"></input><br />这些颜色应该是 6 个字符16值颜色值,无需填写 # 前缀 (例如:000000 为黑色, ffffff 白色)</td></tr>';
	// background color
	echo '<tr valign="top"><th scope="row">背景色</th>';
	echo '<td><input type="text" name="bgcolor" value="'.$options['bgcolor'].'" size="8"></input><br />6 字符16进制颜色值</td></tr>';
	// transparent
	...
	echo '></input><br />选中即打开</td></tr>';
	// speed
	echo '<tr valign="top"><th scope="row">旋转速度</th>';
	echo '<td><input type="text" name="speed" value="'.$options['speed'].'" size="8"></input><br />百分比(无需%符号,初始为100)</td></tr>';
	// distribution
	echo '<tr valign="top"><th scope="row">均匀分布</th>';
	echo '<td><input type="checkbox" name="distr" value="true"';
	if( $options['distr'] == "true" ){ echo ' checked="checked"'; }
	echo '></input><br />均匀分布在球面上,而不是随机排列</td></tr>';
	// end table
	echo '</table>';
	// tags, cats?
	echo '<h3>输出选项</h3>';
	...
	echo '></input> 分类目录<br /><input type="radio" name="mode" value="both"';
	if( $options['mode'] == 'both' ){ echo ' checked="checked" '; }
	echo '></input> 两者都是 (你需要降低标签的数目, 在下面高级设置中配置)';
	// end table
	echo '</table>';
	// advanced options
	echo '<h3>高级设置</h3><p>除非你知道自己在做什么,否则请勿修改!</p>';
	echo '<table class="form-table">';
	// arguments
	echo '<tr valign="top"><th scope="row">wp_tag_cloud 参数</th>';
	echo '<td><input type="text" name="args" value="'.$options['args'].'" size="60"></input><br />wp_tag_cloud 参数代码(请参见更详细的手册 <a href="http://codex.wordpress.org/Template_Tags/wp_tag_cloud#Parameters" target="_blank">codex</a> 英文)<br /><br /><strong>例子</strong><br />number=20 - 标签的最大显示数量<br />smallest=5&largest=50 - 自定义最大和最小的字体<br /><br /><strong>调用说明</strong><br />方式一:从 外观>>小工具 中调用它.<br />方式二:使用函数 <code>< ?php wp_cumulus_insert(); ?></code> 调用到任何位置.</td></tr>';	
	// compatibility mode
	echo '<tr valign="top"><th scope="row">使用兼容模式?</th>';
	echo '<td><input type="checkbox" name="compmode" value="true"';
	if( $options['compmode'] == "true" ){ echo ' checked="checked"'; }
	echo '></input><br />一般不使用,除非你的插件不能正常运行.</td></tr>';
	// show regular tag in alternate content?
	echo '<tr valign="top"><th scope="row">显示普通的 HTML 标签云?</th>';
	...
	echo '</table>';
	echo '<h3>汉化作者</h3><p><strong><a href="http://www.doomeve.com" target="_blank">末日黄昏</a> 提供汉化</strong><ul><li>本插件由 Kisazbis 进行2次开发,为其提供页面汉化和以及字体汉化.如果你想了解更多,欢迎访问我的博客.</li></ul></p>';
