...
<h1>无法建立数据库连接</h1>
<p>这可能是由于您在 <code>wp-config.php</code> 文件中所填写的用户名和密码信息错误，或者 WordPress 无法连接到您所填写的数据库服务器 <code>%s</code>。请参考以下内容检测您的配置是否正确。</p>
<ul>
	<li>您填写的用户名和密码信息；</li>
	<li>您填写的数据库主机名；</li>
	<li>您服务器的数据库运行状况。</li>
</ul>
<p>如果您无法确定这些内容是否正确，请联系您的主机供应商。如果您还需要其他帮助，请访问 <a href='http://wordpress.org/support/'>WordPress Support Forums</a>，中文用户可以访问 <a href='http://wfans.org/forums/'> WordPress 爱好者社区</a>。</p>
"/*/WP_I18N_DB_CONN_ERROR*/, $dbhost ), 'db_connect_fail' );
	...
	function set_prefix( $prefix, $set_table_names = true ) {
		...
		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) )
			return new WP_Error('invalid_db_prefix', /*WP_I18N_DB_BAD_PREFIX*/'无效的数据库前缀'/*/WP_I18N_DB_BAD_PREFIX*/);
...
<h1>无法选择该数据库</h1>
<p>WordPress 可以连接到您的数据库（您所填写的用户名和密码没有问题），但无法选择 <code>%1$s</code> 数据库。</p>
<ul>
<li>您确认这个数据库存在吗？</li>
<li>您确认所填写的用户 <code>%2$s</code> 有足够权限使用 <code>%1$s</code> 数据库吗？</li>
<li>在一些系统中，数据库名称是要包含一个前缀的，于是数据库名可能像这样 username_wordpress，请检查您的数据库名是否存在这样的问题。</li>
</ul>
<p>如果您不知道如何安装一个数据库，那您应该<strong>联系服务器供应商</strong>。如果问题还是无法解决，请登录 <a href="http://wordpress.org/support/">WordPress Support Forums</a> 获取社区帮助，中文用户可以访问 <a href="http://wfans.org/forums/"> WordPress 爱好者社区</a>。</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, $this->dbuser), 'db_select_fail');
			return;
		}
	}
	...
	function print_error( $str = '' ) {
		...
		if ( $caller = $this->get_caller() )
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR_FULL*/'WordPress 数据库错误！在 %3$s 执行 %2$s 时，发生错误： %1$s'/*/WP_I18N_DB_QUERY_ERROR_FULL*/, $str, $this->last_query, $caller );
		else
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR*/'WordPress 数据库错误！执行 %2$s 时，发生错误：%1$s'/*/WP_I18N_DB_QUERY_ERROR*/, $str, $this->last_query );
...
"/*/WP_I18N_DB_CONN_ERROR*/, $details['db_host'] ), 'db_connect_fail' );
	...
	function get_row( $query = null, $output = OBJECT, $y = 0 ) {
		...
		if ( $output == OBJECT ) {
			...
			$this->print_error(/*WP_I18N_DB_GETROW_ERROR*/" \$db->get_row(string query, output type, int offset) -- 输出类型只能是以下几种：OBJECT, ARRAY_A, ARRAY_N"/*/WP_I18N_DB_GETROW_ERROR*/);
		}
	}
